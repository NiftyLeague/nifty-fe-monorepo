...
| Cross-tree ephemeral UI (app navigation drawer and notifications)                         | Provider-scoped `solid-js/store` createStore                        | One store per provider tree; deterministic initial state and reset seam; never shared across SSR requests                            | None                                  | `createMemo` selector accessors prevent unrelated subscribers from rerendering                                                                                                 |
...
| Auth credentials, nonce, UUID, and agreement state                                        | Per-key local-storage vanilla stores read through selector hooks    | Client-only guarded hydration; legacy auth-status read remains bounded                                                               | Existing named local-storage keys     | Raw credentials never enter query keys; query keys use a non-reversible per-session scope                                                                                      |
| Favorites                                                                                 | TanStack Query profile-favorites cache                              | Derives the card-facing list through the query's select; no local-storage mirror                                                     | Memory cache only                     | Optimistic cache write, rollback to the previous cached value on failure, exact cached value pinned on success                                                                 |
...
| Shareable filters, pagination, sorting, leaderboard selection, and rental search/category | Typed URL parsers (`src/url/parsers.ts` + `useQueryStates` adapter) | Typed parsers supply SSR/client defaults; invalid values normalize safely; router search updates                                     | Browser URL/history                   | Push history for discrete choices and pagination; replace history for text search; no parallel store or component copy                                                         |
...
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `app`      | `createStore` for navigation/notifications, TanStack Query for every client-owned remote read and favorite mutation, and local typed parsers for the DEGEN, leaderboard, rental, and mint URL domains | Query client and parsers; remote reads; optimistic favorite mutation; URL consumers; remove legacy loader/bridges; scoped UI stores            | Revert each owner independently. Query and URL changes retain the same HTTP endpoints, response shapes, routes, and storage keys. |
| `smashers` | Retain React local state and PlayFab SWR                                                                                                                                                              | No migration; the one-shot login query token is an auth command, and the launcher-version read has one consumer with no duplicate cache owner  | Existing NextAuth/PlayFab boundary                                                                                                |
| `web`      | Retain server/static route state, component-local interaction, and the isolated claimable-NFTL on-chain read (now rendered through Astro shells; see m0 note)                                         | No migration; the NFTL read has one token-route consumer, aborts on disposal, and `web` has no wallet/query runtime whose cache it could reuse | Existing route modules                                                                                                            |
...
- Favorite updates cancel the matching read, write the profile-favorites cache optimistically, roll back to the previous cached value on failure, and pin the exact cached value on success. Favorites are not mirrored into local storage.
...
- The DEGEN parser owns page, sort, layout, text search, price, tribe, background, cosmetic, token, and wallet filters. Empty/default values are removed from the URL.
- Leaderboard pagination and rental pagination, page size, and sorting use the same typed URL owner as their existing selections and filters. The mint artist override is a typed boolean, so only an explicit `true` enables it.
...
- Consolidated the repeated favorite-DEGEN POST/effect logic from dashboard overview and dashboard DEGENs into one optimistic mutation hook, and removed its `FAV_DEGENS` local-storage mirror together with the auth local-storage mega-context. Auth and identity fields live in per-key local-storage vanilla stores consumed through selector hooks, and the favorites list derives from the profile-favorites query cache.
...
- Removed leaderboard row/count effects, the rental-pass effect, the launcher-version effect/state mirror, and the Merkle module/component caches. Their remaining client reads now use semantic TanStack Query keys, shared retry/cancellation policy, and explicit error surfaces.
...
- Rolling back the Solid stores restores the two context reducers without touching consumers outside navigation/notification providers.
- Local-storage store rollback restores the auth local-storage mega-context and the `FAV_DEGENS` favorites mirror without changing any storage key, value format, or the favorites POST contract.
...
- URL-parser rollback restores one route domain at a time; keep its current query-string spelling so deep links remain compatible.
- Authentication verification/signing, rent/terminate/profile-update/purchase/download actions, transaction gas estimation, and on-chain writes remain commands rather than cached server state. Their local pending/error state is scoped to the initiating interaction.
- Smashers PlayFab SWR, its single-consumer launcher version read, Web's single-consumer abortable claimable-NFTL read, wagmi query ownership, wallet providers, and component-local dialog state are explicit non-migrations, not unfinished M1 work. Adding a second state library or provider to those routes would add runtime and hydration cost without eliminating a duplicate owner.
