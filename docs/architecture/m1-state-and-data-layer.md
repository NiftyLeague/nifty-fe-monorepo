...
- Smashers PlayFab SWR, its single-consumer launcher version read, Web's single-consumer abortable claimable-NFTL read, wagmi query ownership, auth storage, wallet providers, and component-local dialog state are explicit non-migrations, not unfinished M1 work. Adding a second state library or provider to those routes would add runtime and hydration cost without eliminating a duplicate owner.
