...
Bundle and request-count acceptance uses the M0 profile and budgets without reinterpretation. The M1 evidence file records the exact baseline commit, commands, focused behavior results, build outputs, and same-profile local route samples. A route, build, bundle, or request-count regression beyond the M0 budget rejects the migration and rolls back the relevant owner without changing public contracts.
