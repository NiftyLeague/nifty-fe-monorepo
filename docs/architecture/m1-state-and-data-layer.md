...
| `web`      | Retain server/static route state, component-local interaction, and the isolated claimable-NFTL on-chain read                                                                           | No migration; the NFTL read has one token-route consumer, aborts on disposal, and `web` has no wallet/query runtime whose cache it could reuse | Existing route modules                                                                                                            |
