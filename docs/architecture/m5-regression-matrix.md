...
| A11y sweep (axe: labels, contrast)                    | `e2e/a11y.e2e.ts` — zero serious/critical axe floor across all 14 indexable routes; keyboard-only pass over every route (skip link, disclosures, no traps) recorded in `m5-app-audits.md` (M5.5) |
