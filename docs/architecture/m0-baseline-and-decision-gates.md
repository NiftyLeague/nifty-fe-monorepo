...
| `web`      | Astro 7 static + Cloudflare Worker; React islands inside Astro shells          | `/`, `/games`, `/degens`, `/niftyworld`, `/roadmap`, `/gltf/[tokenId]`, invite and party links                | `bun --filter web build` (Astro); Cloudflare Worker `niftyleague.com` |
