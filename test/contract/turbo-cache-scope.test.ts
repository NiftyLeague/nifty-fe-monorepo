...
const sharedBuildInputs: Record<string, string[]> = {
  ...
  'docs#build': [
    // assets/ is every Astro app's publicDir (site chrome only since the media
    // moved to the CDN) and is copied into dist, so an asset edit must
    // invalidate the build even though it lives outside the workspace.
    '../../assets/**',
