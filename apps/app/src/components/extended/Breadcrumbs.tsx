...
const Breadcrumbs = (props: BreadCrumbsProps) => {
  ...
  const iconClass = 'mr-1.5 -mt-0.5 w-4 h-4'
    ...
    >
      {(item) => {
        ...
        return (
          <div
            class={cx(
              props.card === false ? 'mb-0 border-none bg-transparent' : 'mb-6 border bg-background'
            )}
          >
            <div class={cx(props.card === false ? 'py-2 pr-2 pl-0' : 'p-2')}>
              ...
              >
                ...
                <nav aria-label="breadcrumb" class="flex items-center">
                  <Link
                    href="/"
                    class="flex items-center text-sm font-medium text-inherit no-underline"
                  >
                    <Show when={props.icons}>
                      <AppNavIcon name="house" color="blue" fill="dim" class={iconClass} />
                    </Show>
                    <Show when={props.icon}>
                      <AppNavIcon name="house" color="blue" class={cx(iconClass, 'mr-0')} />
                    </Show>
                  ...
                  <Show when={main && main.type === 'collapse' ? main : null} keyed>
                    {(mainItem) => (
                      <>
                        ...
                        >
                          <Show when={props.icons}>
                            <AppNavIcon name={mainItem.icon ?? 'list-tree'} class={iconClass} />
                          </Show>
                          {mainItem.title}
                        </Link>
                      </>
                  ...
                  <span class="mx-1.25 flex w-4 items-center">{separatorIcon()}</span>
                  <span class="flex items-center text-sm font-medium text-muted-foreground no-underline">
                    <Show when={props.icons}>
                      <AppNavIcon name={item.icon ?? 'list-tree'} class={iconClass} />
                    </Show>
                    {item.title}
                  </span>
                </nav>
