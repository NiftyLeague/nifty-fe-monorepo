...
export default function PublicDegenDialog(props: PublicDegenDialogProps) {
  const traits = () => getDegenTraitEntries(props.degen?.traits_string)
        ...
        <div class="grid min-w-0 grid-cols-1 gap-6 md:grid-cols-2">
          <div class="flex min-w-0 flex-col items-center gap-4">
            ...
            <DialogHeader class="items-center">
              <DialogTitle>{props.degen?.name || 'No Name DEGEN'}</DialogTitle>
              <DialogDescription>Degen #{props.degen?.id}</DialogDescription>
              {props.degen?.id && (
                <a
              ...
              {props.degen?.owner && (
                <p class="text-sm text-muted-foreground">
                  Owned by {`${props.degen.owner.slice(0, 5)}...${props.degen.owner.slice(-4)}`}
                </p>
              )}
            </DialogHeader>
          </div>
          <div class="flex min-w-0 flex-col gap-6">
            ...
            <Button
              ...
              autoFocus
              aria-label="Close degen details"
            >
              Close
            </Button>
          </div>
