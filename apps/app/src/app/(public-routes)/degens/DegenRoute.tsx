  ...
  return (
    ...
    >
      ...
      <div aria-hidden="true" className="flex flex-col gap-2 sm:flex-row">
        <DeferredSkeleton className="h-8 flex-1" />
        <div className="flex gap-2">
          <DeferredSkeleton className="h-8 w-36" />
          <DeferredSkeleton className="h-8 w-16" />
        </div>
      </div>
      <div aria-hidden="true" className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {loadingCards.map((index) => (
          <DeferredSkeleton key={index} className="aspect-square w-full rounded-md" />
        ))}
      </div>
    </div>
  )
