...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  ...
  Usage:
  node apple-gen-secret.mjs [--kid] [--iss] [--private_key] [--sub] [--expires_in] [--exp]
    
  Options:
    --help                 Print this help message
    --kid, --key_id        The key id of the private key
    --iss, --team_id       The Apple team ID
    --private_key          File with private key to use to sign the JWT. (Starts with -----BEGIN PRIVATE KEY-----)
  ...
  const txt = fs.readFileSync(private_key, 'utf8');
  const privateKey = txt.replace(/\\n/g, '\n');
