...
- `web`: an [Astro](https://astro.build) static site with a Cloudflare Worker for our company's website [niftyleague.com](http://niftyleague.com)
...
Vercel projects (app, docs, smashers, template) disable Git-triggered feature-branch deployments through each app's `git.deploymentEnabled` and `ignoreCommand` policies. Builds on `main` remain enabled and scoped to affected applications, manual deployments remain available, and the connected projects keep the consolidated Git commit status disabled so ignored builds cannot leave an aggregate check pending. The `web` site is not on Vercel; it builds to static output and deploys as a Cloudflare Worker (`apps/web/wrangler.jsonc`).

## Environment Variables

Environment variables for the Vercel apps (`app`, `docs`, `smashers`, `template`) are managed in **Vercel** (source of truth). Each of those apps is linked to its own Vercel project under the `niftyleague` team. Sync locally:
