...
def transform(text, fname):
    ...
    text = re.sub(
        r"vi\.hoisted\(\s*(\(\)\s*=>.*?)\)\s*;", lambda mm: hoisted_repl(mm) + ";", text, flags=re.S
    )
