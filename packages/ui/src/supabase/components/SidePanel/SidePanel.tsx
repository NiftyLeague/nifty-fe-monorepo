...
const SidePanel = ({
  ...
  const footerClasses = [SlidePanelStyles['sbui-sidepanel-footer-container']];
  if (!customFooter) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      {triggerElement && (
        <Dialog.Trigger className={SlidePanelStyles[`sbui-sidepanel__trigger`]}>{triggerElement}</Dialog.Trigger>
      )}
                  ...
                  <Space size={6} direction="vertical" style={{ minHeight: '0', flex: '1 1 0%', overflowY: 'scroll' }}>
                    <header className={SlidePanelStyles['sbui-sidepanel-header']}>
                      <Space size={3} direction="row" style={{ alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <div className={SlidePanelStyles['sbui-sidepanel-close-container']}>
                          <Button
                      ...
                      <div>{description && <Typography.Text type="secondary">{description}</Typography.Text>}</div>
                    </header>
