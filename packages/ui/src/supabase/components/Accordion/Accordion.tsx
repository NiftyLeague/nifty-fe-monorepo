...
interface AccordionProps {
  ...
  bordered?: boolean;
  onChange?: (item: { label: string; id?: string | number; open: boolean }) => void;
}
...
function Accordion({
  ...
  const containerClasses = [AccordionStyles['sbui-accordion-container']];
  if (bordered) {
...
export function Item({ children, className, label, id }: ItemProps) {
  const { defaultActiveId, icon, iconPosition, onChange } = useContext(AccordionContext);

  const panelClasses = [AccordionStyles['sbui-accordion-item__panel']];

  const buttonClasses = [AccordionStyles['sbui-accordion-item__button']];
  if (className) {
