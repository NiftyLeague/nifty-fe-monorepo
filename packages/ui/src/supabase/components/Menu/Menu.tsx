...
function Menu({ children, className, style }: MenuProps) {
  return (
    <div role="menu" aria-orientation="vertical" aria-labelledby="options-menu" className={className} style={style}>
      {children}
    </div>
  );
}
  ...
  return (
    ...
    <DropdownContext.Consumer>
      {({ onClick }) => {
        ...
        return (
          <div className={classes.join(' ')} role="menuitem" onClick={handleClick} style={style}>
            <Typography.Text>
              <Space>
                {icon && icon}
