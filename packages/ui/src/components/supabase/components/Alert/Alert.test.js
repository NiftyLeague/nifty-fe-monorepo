...
describe('#Alert', () => {
  ...
  it('should render an icon when passed withIcon', () => {
    ...
    expect(container.querySelector('div.flex-shrink-0 > svg')).toBeInTheDocument();
  });

  it.each(VARIANTS)('should have "sbui-alert-[container|description]--%s" class', variant => {
    const { container } = render(
    ...
    expect(container.querySelector('div')).toHaveClass(`sbui-alert-container sbui-alert-container--${variant}`);
    expect(screen.queryByText('Description')).toHaveClass(`sbui-alert-description sbui-alert-description--${variant}`);
  });
});
