...
function ContextMenu({
  ...
  const classes = [ContextMenuStyles['sbui-contextmenu__content']];
  if (className) {
    classes.push(className);
  }
  return (
    <RadixContextMenu.Root onOpenChange={onOpenChange}>
      <RadixContextMenu.Trigger className={ContextMenuStyles['sbui-contextmenu__trigger']}>
        {children}
      </RadixContextMenu.Trigger>

      <RadixContextMenu.Content alignOffset={alignOffset} className={classes.join(' ')} style={style}>
        {overlay}
...
export function Label({ children }: LabelProps) {
  return (
    <RadixContextMenu.Label className={ContextMenuStyles['sbui-contextmenu-label']}>{children}</RadixContextMenu.Label>
  );
}
