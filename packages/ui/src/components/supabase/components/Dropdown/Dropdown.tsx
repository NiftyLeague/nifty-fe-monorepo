...
function Dropdown({
  ...
  return (
    <RadixDropdown.Root onOpenChange={onOpenChange} open={open}>
      {isNested ? (
        <RadixDropdown.Trigger className={DropdownStyles['sbui-dropdown__trigger-item']}>
          {children}
        </RadixDropdown.Trigger>
      ) : (
        <RadixDropdown.Trigger className={DropdownStyles['sbui-dropdown__trigger']}>{children}</RadixDropdown.Trigger>
      )}
      ...
      >
        {arrow && (
          <RadixDropdown.Arrow className={DropdownStyles['sbui-dropdown__arrow']} offset={10}></RadixDropdown.Arrow>
        )}
        {overlay}
      </RadixDropdown.Content>
    </RadixDropdown.Root>
...
export function Item({ children, icon, disabled, onClick }: ItemProps) {
  return (
    <RadixDropdown.Item className={DropdownStyles['sbui-dropdown-item']} disabled={disabled} onSelect={onClick}>
      {icon && icon}
      <span>{children}</span>
    </RadixDropdown.Item>
  );
...
export function Label({ children }: LabelProps) {
  return <RadixDropdown.Label className={DropdownStyles['sbui-dropdown-label']}>{children}</RadixDropdown.Label>;
}
