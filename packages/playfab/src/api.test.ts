...
describe('PlayFab account API', () => {
  it('registers and logs in users with normalized requests', async () => {
    ...
    await expect(api.RegisterPlayFabUser({ Email: 'player@example.com', Password: 'secret' })).resolves.toEqual({
      PlayFabId: 'player',
    });
    await expect(api.LoginWithEmailAddress({ Email: 'player@example.com', Password: 'secret' })).resolves.toEqual({
      SessionTicket: 'email-ticket',
    });
    await expect(api.LoginWithCustomID({ CustomId: 'custom' })).resolves.toEqual({ SessionTicket: 'custom-ticket' });
...
describe('provider linking', () => {
  it.each([
    ...
    await expect(api.LinkProvider(provider, 'access-token', 'ticket')).resolves.toEqual({ data: { linked: provider } });
  ...
  it.each([
    ...
    await expect(api.UnlinkProvider(provider, 'ticket')).resolves.toEqual({ data: { unlinked: provider } });
  });
...
describe('profile and publisher data', () => {
  it('reads and updates account data', async () => {
    ...
    await expect(api.GetAccountInfo('ticket')).resolves.toHaveProperty('AccountInfo.PlayFabId', 'player');
    ...
    await expect(api.GetUserPublisherData('ticket')).resolves.toHaveProperty('DisplayName.Value', 'Nifty');
...
describe('admin and cloud functions', () => {
  it('deletes players and changes display names', async () => {
    ...
    await expect(api.ChangeDisplayName('Nifty', 'entity-token')).resolves.toEqual({ FunctionResult: 'ok' });
  });
  ...
  it('links and unlinks Ethereum wallets through cloud functions', async () => {
    ...
    await expect(
      api.UnlinkWallet({ address: '0xABCDEF012345', EntityToken: 'entity-token', SessionTicket: 'ticket' }),
    ).resolves.toEqual({ FunctionResult: 'linked' });
  });
